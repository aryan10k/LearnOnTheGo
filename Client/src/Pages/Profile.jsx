...
const Profile = () => {
    ...
    const submitHandler = async(e)=>{
       ...
       try {
        setLoading(true)
        const res = await axios.put('https://learnonthego.onrender.com/api/v1/user/profile/update',formData, {
            headers:{
        ...
        <div className='bg-gray-100 py-12 px-4 lg:px-0'>
            <div className='max-w-6xl mx-auto p-8 bg-gradient-to-r bg-white shadow-xl rounded-2xl mt-14'>
                <div className='flex flex-col items-center md:flex-row md:items-start space-y-8 md:space-y-0 md:space-x-12'>
                    {/* profile picture */}
                    <div className='w-40 h-40 rounded-full overflow-hidden border-4 border-green-600 shadow-lg'>
                        <img src={user?.photoUrl || UserLogo} alt="" className='w-full h-full object-cover' />
                    </div>

                    {/* user info */}
                    <div className='text-center md:text-left'>
                        <h1 className='text-4xl font-bold text-green-600'>Welcome, {user?.name.split(" ")[0] || "User"} </h1>
                        ...
                        <Dialog open={open} onOpenChange={setOpen}>
                            <Button onClick={()=>setOpen(true)} className="bg-green-600">Edit Profile</Button>
                            <DialogContent className="sm:max-w-[425px]">
                                ...
                                <DialogFooter>
                                    {
                                        loading ? <Button className="bg-green-400"><Loader2 className='mr-2 w-4 h-4 animate-spin'/> Please wait</Button> : <Button
                                        onClick={submitHandler} className="bg-green-500">Save Changes</Button>
                                    }
